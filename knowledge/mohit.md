## 1. Profile and Positioning

Mohit Singhal is an AI and Automation Engineer focused on Generative AI, Large Language Models (LLMs), RAG, AI agents, agentic systems, and intelligent automation.

He is based in Noida, Uttar Pradesh, India, in the Delhi-NCR region. His hometown is Rishikesh, Uttarakhand, India.

Mohit's primary professional focus is building AI-powered software and automation solutions that connect LLMs, AI agents, enterprise systems, APIs, and business workflows.

He works with business and technical stakeholders to understand requirements, identify repetitive or manual processes, and develop department-specific automations that reduce manual work.

Examples of department-specific automation work include:
- HR chatbots and HR query automation
- Email lead sorting and qualification agents
- Marketing content generation agents
- Customer support automation
- Employee assessment automation
- Policy and knowledge management automation

### Primary Professional Expertise

- Generative AI
- Large Language Models (LLMs)
- Retrieval-Augmented Generation (RAG)
- AI agents
- Multi-agent systems
- Agentic AI
- Conversational AI
- Intelligent automation
- Enterprise AI
- LangChain
- LangGraph
- LangSmith
- LLM evaluation
- RAG evaluation
- Prompt engineering
- LLM orchestration

### Availability 

Mohit is open to:
- Full-time AI and Automation opportunities (Immediate Joiner)
- Generative AI / LLM / RAG roles
- AI automation projects
- Freelance work
- Project-based or consulting work


## 2. Professional Experience

### Software Engineer — CodeStore Technologies Pvt. Ltd.

**Duration:** June 2025 – June 2026
**Location:** Noida, India
**Career progression:** Software Engineer Trainee → Software Engineer
**Work arrangement:** On-site
**Status:** Not Working (Served Notice/Immediate Joiner)

Mohit's professional work includes AI-powered software, enterprise automation, backend services, REST APIs, full-stack applications, cloud platform services, and scalable automation infrastructure.

He works with business stakeholders and technical teams to:
- Gather and understand business requirements
- Identify manual or repetitive workflows
- Design AI and automation solutions for specific departments
- Integrate APIs, internal tools, and business systems
- Build and maintain automation infrastructure
- Improve reliability, monitoring, deployment efficiency, and operational performance

His work includes:
- n8n for AI Automation workflows
- Microsoft Copilot Studio agents for HR and departmental use cases
- AI-powered full-stack applications
- Backend services and REST APIs
- Automation pipelines and workflow automation
- Scalable microservices
- Dashboards and web scraping applications
- Microsoft 365, Teams, Outlook, Zoho, and Azure DevOps integrations
- AI research and proof-of-concept development
- Software validation, QA testing, troubleshooting, and technical documentation

## 3. AI, Generative AI, RAG, and Agents

### Generative AI and LLMs

Mohit works with LLM applications for:
- Customer support
- Lead qualification and classification
- Employee assessment
- Research and research report generation
- Policy-grounded responses
- AI-generated performance summaries
- Agentic workflows
- Enterprise automation

Documented model usage includes:
- GPT-4.1-mini
- Gemini 2.5 Flash
- GPT-4o in a portfolio code example

### Retrieval-Augmented Generation (RAG)

Mohit has hands-on experience building RAG systems.

His RAG work includes:
- Documentation chunking
- Embedding generation
- Semantic retrieval
- Vector database usage
- Contextual answer generation
- Product knowledge retrieval
- Policy knowledge retrieval
- Policy-grounded responses
- Knowledge management workflows

Major RAG projects include:
- AI-WhatsApp Customer Support Agent
- AI-Powered Employee Assessment Agent

### LangChain

Mohit uses LangChain for:
- LLM application development
- Agent development
- Agent orchestration
- Tool-using agents
- LLM orchestration
- LCEL-based workflows
- RAG-related applications

### LangGraph

Mohit uses LangGraph for agentic AI and multi-agent workflow orchestration.

The DeepResearch Agent uses LangGraph for its multi-stage research workflow.

### AI Agents and Multi-Agent Systems

Mohit's AI agent work includes:
- Customer Support Agent
- Follow-up Agent
- Lead Qualification Agent
- Search Agent
- Reader Agent
- Writer Chain
- Critic Chain
- Employee Assessment Agent
- HR Policy Agent
- Department SOP Agent Network
- Email Qualification Agent
- Workforce Activity Analysis Agent
- Automated News & Social Media Agent
- YouTube Research & Content Ideation Agent
- LinkedIn Content Research Agent

His agentic work spans customer support, lead qualification, employee assessment, policy knowledge management, research, and enterprise automation.

## 4. LangSmith, LLM Evaluation, and RAG Evaluation

Mohit uses LangSmith for LLM tracing and observability.

His LangSmith work includes:
- Tracing application runs
- Inspecting retrieval and generation steps
- Monitoring latency
- Monitoring token usage
- Monitoring LLM costs
- Inspecting prompts, retrieved context, and model outputs
- LLM evaluation
- RAG evaluation
- Evaluating grounding and hallucination
- Evaluating answer relevance and correctness

His portfolio chatbot is instrumented with LangSmith to trace retrieval and streaming generation and to measure token usage, latency, and cost.

## 5. AI and Business Automation

Mohit's automation work focuses on connecting AI decision-making with business workflows, APIs, enterprise tools, CRM systems, and notification systems.

Examples include:

### HR Automation
- HR chatbots
- HR query automation
- HR policy agents
- Department-specific HR workflows
- Employee assessment automation

### Lead and Sales Automation
- Email lead sorting
- Lead qualification agents
- Lead classification
- Customer follow-up workflows

### Marketing Automation
- Marketing content generation agents
- Research and content ideation workflows
- Social media automation

### Enterprise Automation
- Workflow automation pipelines
- API and business-system integrations
- Microsoft 365, Teams, Outlook, Zoho, and Azure DevOps integrations
- Enterprise knowledge and policy workflows

Mohit works with stakeholders to understand business requirements and translate them into practical AI and automation workflows that reduce manual work.

## 6. Professional Projects

### AI-WhatsApp Customer Support Agent

A multi-agent AI system for customer support, follow-ups, and lead qualification over WhatsApp.

Key components:
- Customer Support Agent
- Follow-up Agent
- Lead Qualification Agent
- RAG pipeline
- Shared conversation context
- Dynamic follow-up workflows
- Human handoff
- Conversation monitoring
- Validation
- Security and reliability controls
- Error alerting

Technical stack:
- n8n
- PostgreSQL
- Pinecone
- WATI API
- Webhooks
- GPT-4.1-mini
- Google Sheets
- Gmail
- Telegram
- text-embedding-3-large

RAG pipeline:
1. Documentation chunking
2. Embedding generation
3. Semantic retrieval
4. Retrieval-based product query resolution

MCP Server is also used in this project for certain AI/tool integration flows.

Reported outcomes:
- Approximately 25% reduction in manual lead qualification effort
- Approximately 40% reduction in support costs

### AI-Powered Employee Assessment Agent

An enterprise RAG-based assessment and compliance platform using organizational knowledge and departmental documentation.

Key components:
- Microsoft Copilot Studio
- Power Automate
- GPT-4.1-mini
- Azure DevOps Wiki
- Microsoft Graph APIs
- Zoho People
- React
- Power Apps
- REST APIs
- Teams and Outlook

The system:
- Uses RAG over organizational knowledge
- Performs semantic retrieval
- Generates contextual answer explanations
- Produces policy-grounded responses
- Creates dynamic assessments
- Integrates HR and CRM systems
- Generates AI-powered performance summaries
- Supports compliance reporting and knowledge tracking

### DeepResearch Agent

An autonomous multi-agent research pipeline built with Python, LangChain, LangGraph, Gemini 2.5 Flash, Tavily, BeautifulSoup, Streamlit, and LCEL.

Four-stage workflow:
1. Search Agent
2. Reader Agent
3. Writer Chain
4. Critic Chain

It performs:
- Web search
- Web scraping
- Source retrieval
- Research analysis
- Research synthesis
- Report generation
- Critique
- Autonomous task orchestration

This project is an agentic research system and is not presented as a Pinecone-based RAG project.

## 7. Side and Hobby Projects

The following projects are side or hobby projects and should be treated as project-based exposure rather than primary professional specialization.

### Algorithmic Trading Platform

An in-development personal project exploring:
- Real-time market data
- Automated order execution
- Position management
- Risk management
- Real-time processing
- Distributed backend services
- Cloud deployment

Technologies explored:
- Python
- FastAPI
- Flask
- REST APIs
- WebSockets
- AsyncIO
- PostgreSQL
- OAuth 2.0
- Protocol Buffers
- Azure Virtual Machines
- Linux / Ubuntu
- Nginx
- Git / GitHub
- Cron Jobs
- Upstox Broker APIs

These technologies represent project-level exposure and working knowledge gained through personal experimentation.

### Quantitative Strategy Research and Backtesting

A personal research project exploring systematic trading strategies using Python, Pandas, and NumPy.

It includes:
- Historical market data analysis
- Strategy research
- Strategy design
- Backtesting
- Strategy optimization
- Transaction-level evaluation
- Execution quality analysis
- Risk management analysis

Strategies explored include:
- Range Breakout
- ATR-based momentum
- Market Depth analysis
- Option buying
- Option selling
- Iron Condor
- Straddle
- Strangle
- Hedging strategies

This is a side research project and should not be interpreted as primary professional data-science or quantitative-finance expertise.

## 8. Technologies and Tools

### Primary Professional Technologies

- Python
- LangChain
- LangGraph
- LangSmith
- RAG
- AI Agents
- Prompt Engineering
- LLMs
- n8n
- Microsoft Copilot Studio
- Power Automate
- Power Apps
- React
- REST APIs
- Webhooks
- PostgreSQL
- Pinecone
- Microsoft Azure
- Azure DevOps
- Microsoft Graph APIs
- Zoho People
- WATI API

### Additional Project-Based / Side-Project Technologies

These technologies come primarily from side or hobby projects:
- FastAPI
- Flask
- WebSockets
- AsyncIO
- OAuth 2.0
- Protocol Buffers
- Azure Virtual Machines
- Linux
- Nginx
- Pandas
- NumPy
- Streamlit
- BeautifulSoup
- Tavily API
- MobaXterm
- Cron Jobs

### MCP Server

Mohit has used MCP Server in the AI-WhatsApp Customer Support Agent and in certain flows across other automation projects to connect AI systems with tools and external services.

### AWS

AWS fundamentals are part of Mohit's cloud learning background. He has completed AWS Certified Cloud Practitioner training through Udemy. This should not be interpreted as production AWS experience.

## 9. Education

### B.Tech in Computer Science and Engineering — Uttaranchal University
- Duration: 2021 – 2025
- Location: Dehradun, Uttarakhand
- CGPA: 8.8

### Senior Secondary School — Nirmal Ashram Deepmala Public School
- Duration: 2020 – 2021
- Location: Rishikesh, Uttarakhand
- Percentage: 83%

### High School — Nirmal Ashram Deepmala Public School
- Duration: 2018 – 2019
- Location: Rishikesh, Uttarakhand
- Percentage: 91.8%

## 10. Certifications and Achievements

### Microsoft Certified: Azure AI Fundamentals (AI-900)
- Organization: Microsoft
- Status: Certified

### AWS Certified Cloud Practitioner Training
- Provider: Udemy
- Status: Training

### Selected Outcomes
- Microsoft Copilot Studio agents for HR and departmental use cases helped automate routine HR queries by approximately 30%.
- AI-WhatsApp Customer Support Agent reduced manual lead qualification effort by approximately 25%.
- AI-WhatsApp Customer Support Agent reported a 40% reduction in support costs.

## 11. Technology and Experience Classification

When answering questions about Mohit's skills, distinguish between primary professional expertise and project-based exposure.

### Primary Professional Expertise
- Generative AI
- LLM applications
- RAG
- AI agents
- Multi-agent systems
- Intelligent automation
- Enterprise AI
- LangChain
- LangGraph
- LangSmith
- LLM evaluation
- RAG evaluation
- Prompt engineering
- Business requirements and stakeholder collaboration

### Professional Project Experience
- Microsoft Copilot Studio
- Power Automate
- Power Apps
- React
- n8n
- REST APIs
- Webhooks
- PostgreSQL
- Pinecone
- Azure
- Azure DevOps
- Microsoft 365
- Teams
- Outlook
- Zoho
- Microsoft Graph APIs
- WATI API

### Side / Hobby Project Experience
- FastAPI
- Flask
- WebSockets
- AsyncIO
- OAuth 2.0
- Protocol Buffers
- Azure Virtual Machines
- Linux
- Nginx
- Pandas
- NumPy
- Streamlit
- MobaXterm
- Cron Jobs

Side-project technologies should be described as project-level exposure or working knowledge, not automatically as deep professional specialization.

### Technologies Not Established as Production Experience
- AWS Lambda
- Kubernetes
- Other technologies not explicitly supported by the knowledge base

Do not claim professional experience with a technology unless the knowledge base supports it.

## 12. Career Interests and Work Preferences

Mohit is interested in:
- AI Engineering
- AI and Automation Engineering
- Generative AI
- LLM applications
- RAG systems
- Agentic AI
- Enterprise AI
- Intelligent automation
- Complex system architectures
- Scalable AI systems

Mohit is open to:
- Full-time opportunities
- AI and automation roles
- Freelance work
- Project-based work
- Consulting work

## 13. Professional Strengths

- Analytical thinking
- Problem-solving
- Algorithmic thinking
- Stakeholder collaboration
- Teamwork
- Adaptability
- Technical documentation
- Requirements gathering
- Technical and business stakeholder communication
- Cross-functional collaboration
- Agile software development
- Software validation
- QA testing
- Troubleshooting
- Monitoring
- Reliability engineering
- Performance optimization
- Deployment efficiency
- Operational improvement
